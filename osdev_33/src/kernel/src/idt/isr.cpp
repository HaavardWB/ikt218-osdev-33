...
namespace UiAOS::CPU::ISR {
    ...
    void print_interrupt_message(registers_t* regs, void* context) {
        // Assuming interrupt number is what you want to print
        uint8_t int_no = regs->int_no;

        // Check the interrupt number
        if (int_no == ISR1) {
            write_string(5,30,9,"interrupt 1 triggered");
        } else if (int_no == ISR2) {
            write_string(5,30,9,"interrupt 2 triggered");
        }
    }
    ...
    // Register print_interrupt_message as the ISR for interrupt number 123
    UiAOS::CPU::ISR::register_interrupt_handler(123, print_interrupt_message, nullptr);
    }
}
